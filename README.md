# 🛡️ Medieval Knights  
*A 24-hour game-jam platformer*

![Platformer](https://img.shields.io/badge/Genre-Platformer-blue)
![Status](https://img.shields.io/badge/Build-Complete-brightgreen)
![Time](https://img.shields.io/badge/Dev%20Time-24%20Hours-orange)
![Engine](https://img.shields.io/badge/Engine-Your%20Engine%20Here-lightgrey)

---

## 🎮 Overview
**Medieval Knights** is a fast-paced 2D platformer created in just one day for a game jam. You control a lone knight navigating four handcrafted levels filled with tight platforming, hazards, enemies, and simple but satisfying movement.

The game is short, replayable, and designed to deliver the classic arcade feel in a compact experience.

---

## ✨ Features
- ⚔️ Four handcrafted levels  
- 🌀 Fast, responsive movement  
- 💀 Enemies and environmental hazards  
- 🏹 Simple sword combat  
- 🎨 Minimalist medieval aesthetic  
- ⏱️ Perfect for speedruns and quick sessions  

---

## 📷 Screenshots
> *(Add your actual images later!)*

---

## 🕹️ Controls

| Action     | Key |
|------------|-----|
| Move       | Arrow Keys / A & D |
| Jump       | Space |
| Attack     | *Your attack key here* |
| Restart    | R (optional) |

---

## 🧩 Gameplay
Master timing, momentum, and precision as each level increases in difficulty. Tight jumps, enemy placements, and simple combat keep the challenge rising without overwhelming the player.

Medieval Knights is built for short sessions but rewards consistent improvement and quick reactions.

---

## 🛠️ Tech Stack
- **Engine:** *Godot*  
- **Languages:** *GDScript*  
- **Assets:** *Taken from itch.io*

---

## 📦 Installation

1. Download the latest release from the Releases tab.
2. Extract the zip file.
3. Run the executable for your platform.

---

## 🧰 Known Limitations
Since this game was made in 24 hours, some mechanics and visuals are intentionally simple. Future updates may expand levels or polish mechanics.

---

## 👤 Credits
**Developer:** Dhami  
**Art & Sound:** *(Add sources or mark as self-made)*  
**Special Thanks:** Game jam organizers & playtesters  

---

## ⭐ Support the Project
If you enjoyed the game, consider starring the repo!

```

